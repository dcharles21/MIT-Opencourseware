...
class WeightedEdge(Edge):
    ...
    def __str__(self):
        ...
        return edge.__str__() + ' ' + '({}, {})'.format(self.total_distance, self.outdoor_distance)
...
class Digraph(object):
    ...
    def add_edge(self, edge):
        ...
        if not (src in self.nodes and dest in self.nodes):
            raise ValueError('the node was not added. Hmph')

        self.edges[src].append(dest)
        self.weights[src, dest] = (total_distance, outdoor_distance)
        ...
        for src in self.edges:
            for dest in self.edges[src]:
                result.append(src.get_name() + '->'\
                         + dest.get_name() + ' ' + \
                             str(self.weights[src, dest]))                                                              

        result = sorted(result)

        return '\n'.join(result) #omit final newline
