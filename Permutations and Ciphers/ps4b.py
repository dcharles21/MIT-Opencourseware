...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        for char in lower + upper:
            ...
            if char in lower:
                value = (lower.find(char) + shift) % 26
                dict[char] = lower[value]             

            if char in upper:
                value = (upper.find(char) + shift) % 26
                dict[char] = upper[value]
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        c = Message(self.message_text)
        words = c.get_message_text()  

        for i in range(0, 26):

            count = 0
            words = c.apply_shift(26-i)

            for w in words.split():

                if is_word(c.get_valid_words(), w):
                    count += 1

            keys[count] = (words, 26-i)        
